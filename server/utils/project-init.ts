...
**IMPORTANTE**: Este loop NO es opcional. Seguilo exactamente según tu plataforma.

### Reactive Wakeup (Antigravity)
\`\`\`
LISTEN_MODE=reactive node .agents/skills/codehive-protocol/listener.js &
→ Listener background forever. Reactive Wakeup te despierta.
→ chat_read → procesar → responder → chat_read recovery → loop
\`\`\`
...
1. chat_read → procesar → responder → chat_read recovery
── FASE DE ESPERA ──
2. Dormir (Reactive Wakeup te despierta)
\`\`\`
